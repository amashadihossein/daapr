...
After adding code, you can choose to follow the steps in `dp_journal.RMD` 
instead of following the rest of these steps. 
...
- If CI/CD is set up, the build process should pick up the deployment from here. 
Specifically, remote server will clone the code, build the data product and deploy 
it to the remote location as specified by the configuration set up during the initialization. 

- In the absence of CI/CD, while GitHub push can be skipped, avoid doing so. 
First push the code to the repo and then run `dpdeploy::dp_deploy(<project path>)` 
to deploy the code and metadata representing the data.
